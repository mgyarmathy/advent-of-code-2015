...
class Lights:
    ...
    def __init__(self, rows, columns):
        ...
        self.lights = rows * columns * bitarray('0')
    ...
    def apply_instruction(self, instr):
        ...
        for idx in range(num_rows):
            ...
            if instr.action == LightInstructionAction.ON:
                self.lights[idxA:idxB] = True
            elif instr.action == LightInstructionAction.OFF:
                self.lights[idxA:idxB] = False
            elif instr.action == LightInstructionAction.TOGGLE:
                self.lights[idxA:idxB] = ~self.lights[idxA:idxB]
            else:
                pass

    def on_count(self):
        return self.lights.to01().count('1')

    def __str__(self):
        return '\n'.join([self.lights[row*self.columns:((row+1)*self.columns)].to01() for row in range(self.rows)])
...
def main():
    ...
    print 'number of lights lit: ' + str(lights.on_count())

def run_tests():
    lights = Lights(5, 5)
    assert lights.on_count() == 0
